...
func NewClient(nodeURL, username, password string, isPi bool) *Client {
	...
	blockCache, _ := lru.New(cacheSize)
	txDataCache, _ := lru.New(cacheSize)
	keyToTxCache, _ := lru.New(cacheSize)
...
func (c *Client) doPost(endpoint string, payload any) (*http.Response, error) {
	...
	if payload != nil {
		...
		c.rpcMu.Lock()
		resp, reqErr = c.httpClient.Post(c.RPCURL+endpoint, "application/json", reqBody)
		c.rpcMu.Unlock()
	} else {
		c.rpcMu.Lock()
		resp, reqErr = c.httpClient.Post(c.RPCURL+endpoint, "application/json", nil)
		c.rpcMu.Unlock()
	}
